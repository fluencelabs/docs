...
- **`status`**: Current operational status of the VM. Statuses:
  ...
  - `Terminated`: VM was terminated by the provider. The VM is inaccessible, and your workload is no longer running. For detailed information, please see the dedicated section on [Handling Provider-Terminated VMs](../../../cpu_cloud/manage_vm/provider_vm_termination.md).
...
SSH keys are shared across CPU and GPU Cloud. See [Manage SSH keys](../../ssh_keys/ssh_keys.md) for the full API reference on listing, adding, and removing SSH keys.
