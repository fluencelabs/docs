...
A long-running agent can create an API key once and send it in the `X-API-KEY` header instead of refreshing tokens. This example gives the key every permission your account has, which covers renting both CPU and GPU compute:

```sh
SCOPES=$(curl -s https://api.fluence.dev/v1/users/me \
  -H "Authorization: Bearer <ACCESS_TOKEN>" | jq -c '.permissions')

curl -X POST https://api.fluence.dev/v1/api_keys \
  -H "Authorization: Bearer <ACCESS_TOKEN>" \
  -H "Content-Type: application/json" \
  -d "{\"name\": \"my-agent\", \"scopes\": $SCOPES, \"expiresAt\": \"2027-01-01T00:00:00Z\"}"
```

- `name`: lowercase letters, digits and hyphens, up to 25 characters.
- `scopes`: what the key may do; a key can only get permissions your account has, listed in `permissions` by `GET /v1/users/me`. A call outside the key's scopes returns `403`. GPU Cloud does not accept keys limited to read-only scopes.
...
4. Register your SSH public key: `POST /v1/ssh_keys`.
...
- If the balance runs out and the debt stays unpaid for 3 days or exceeds 5 USD, your VMs and public IPs are removed; if the debt reaches 20 USD or is still unpaid 10 days later, your disks are deleted as well.

### GPU instances

A wallet account can rent GPU containers, VMs and bare metal as well; see the [GPU Cloud API](./gpu_cloud.md). GPU instances are billed differently from CPU VMs: hourly, with a few hours of rent reserved from your balance at deployment. See [GPU Cloud billing](../gpu_cloud/overview.md#billing-model).
