...
![Running Instances page with a banner notification and a terminated instance](./assets/terminated_vms_general.png)

**Filtering.** You can use the status filter on the Running Instances page and select `Terminated` to view only these instances.

![Running Instances page with a status filter](./assets/terminated_vms_filtered.png)
...
Click **Terminate** on the instance with `Terminated` status to remove its record and release its reserved funds. See [instance management](./manage_vm.md#instance-management) for details.
