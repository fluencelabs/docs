...
* provider's **limit** - a **resource** can have at most 32 providers. Each new provider added after the provider limit has been reached results in removing an old provider following the FIFO principle. Soon provider's prioritization will be handled by TrustGraph.
...
* **resource** and provider **lifetime** - a **resource** and **provider record** are republished every [1 hour](https://github.com/fluencelabs/registry/blob/a89c7c68506dc144ebee208f172bd086ce318b18/service/src/defaults.rs#L23) and evicted, i.e. removed, [24 hours](https://github.com/fluencelabs/registry/blob/a89c7c68506dc144ebee208f172bd086ce318b18/service/src/defaults.rs#L24) after being unused.
...
:::info
There are [several simple examples](https://github.com/fluencelabs/registry/blob/721264846c6f16b8f366c269c834bb40c80e88eb/README.md#how-to-use-it-in-aqua) in the `fluencelabs/registry` repo. Give them a look.
:::
...
func call_provider(p: Record):
    ...
    on p.peer_id via p.relay_id:
        ...
        -- call function
        ProviderApi.do_something(p.value)

-- call ProviderApi.do_something() on every provider
...
That set is called "neighborhood" or "K-closest nodes" (K=20). In Aqua, it is accessible in `aqua-lib` via the `Kademlia.neighborhood` function.
  ...
  on HOST_PEER_ID:
    ...
    if record_sig_result.success == false:
      ...
      if key == nil:
        ...
        successful: *bool
        -- get neighborhood for the resource_id
        nodes <- getNeighbours(resource_id)
        -- iterate through each node in the neighborhood
        for n <- nodes par:
          error <<- n
          on n:
            try:
              ...
              if republish_res.success == false:
                ...
                -- register as a provider on each node in the neighborhood
