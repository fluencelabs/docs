...
function cleanMarkdownForDisplay(content, filepath) {
  ...
  content = content.replace(
    ...
    (match, alt, relPrefix, filename) => {
      // Convert to absolute path: /docs/path/to/file/assets/filename
...
async function copyImageDirectories(docsDir, buildDir) {
  ...
  // Recursively find all 'assets' directories in docs
  function findImgDirs(dir, baseDir = dir) {
    ...
    files.forEach((file) => {
      ...
      if (stat.isDirectory()) {
        if (file === 'assets') {
          // Found an assets directory, store its relative path
  ...
  // Copy each assets directory to build
  let copiedCount = 0;
  for (const { source, relativePath } of imageDirs) {
    const destination = path.join(buildDir, 'docs', relativePath, 'assets');

    try {
      await fs.copy(source, destination);
      const imageCount = fs.readdirSync(source).length;
      console.log(`  ✓ Copied: ${relativePath}/assets/ (${imageCount} images)`);
      copiedCount++;
    } catch (error) {
      console.error(`  ✗ Failed to copy ${relativePath}/assets/:`, error.message);
    }
  }
  ...
  return {
    ...
    async postBuild({ outDir }) {
      ...
      for (const mdFile of mdFiles) {
        ...
        const destPath = path.join(buildDir, 'docs', mdFile);
